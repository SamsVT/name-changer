...
public class Name_changerClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        ClientReceiveMessageEvents.ALLOW_CHAT.register((Text message,
            ...
            if (isHideAll()) {
                Text shownName = Text.translatable("text.name_changer.hidden_name");
                Text newMsg = Text.translatable("chat.type.text", shownName, content);
                MinecraftClient.getInstance().execute(() -> MinecraftClient.getInstance().inGameHud.getChatHud().addMessage(newMsg));
                return false;
            }
